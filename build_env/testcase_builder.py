...
def copy_makefiles(hemps_path, testcase_path, page_size_KB, memory_size_KB, system_model_description, apps_list, simul_time):
    ...
    lines.append("\n" + "PAGE_SP_INIT = "+ str((page_size_KB  *  1024)) + "\n") #TODO: verificar a geracao de sp
    # lines.append("PAGE_SP_INIT = "+ str((page_size_KB  *  1024) - 1) + "\n") 
    # lines.append("PAGE_SP_INIT = 0x10000\n")
    # lines.append("MEM_GP_INIT = "+ str(((page_size_KB + 2)  *  1024))   + "\n") #Set 4k gp (center 4k/2) after the end of memory - usando --no-relax no ld
    # lines.append("MEM_SP_INIT  = "+ str((memory_size_KB * 1024) - 1) + "\n")
    lines.append("\n" + "MEM_SP_INIT  = "+ str((memory_size_KB * 1024)) + "\n")
