...
def main():
    ...
    exit_status = os.system("cd applications/; make sp_init=" + str(page_size_bytes) )
...
def get_task_DATA_size(app_name, task_name):
    ...
    data_size = int (commands.getoutput("riscv32-unknown-elf-size "+source_file+" | tail -1 | sed 's/ //g' | sed 's/\t/:/g' | cut -d':' -f2"))
...
def get_task_BSS_size(app_name, task_name):
    ...
    #https://www.quora.com/What-is-a-convenient-way-to-execute-a-shell-command-in-Python-and-retrieve-its-output
    bss_size = int(commands.getoutput("riscv32-unknown-elf-size "+source_file+" | tail -1 | sed 's/ //g' | sed 's/\t/:/g' | cut -d':' -f3"))
