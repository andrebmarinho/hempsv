...
void write_local_msg_to_task(TCB * task_tcb_ptr, int msg_lenght, int * msg_data){
	...
	msg_ptr = (Message*)((task_tcb_ptr->offset) | ((unsigned int)task_tcb_ptr->reg[9])); //reg[9] = address message
...
int Syscall(unsigned int service, unsigned int arg0, unsigned int arg1, unsigned int arg2) {
	...
	switch (service) {
		...
		case WRITEPIPE:
			...
			//puts("WRITEPIPE - prod: "); puts(itoa(producer_task)); putsv(" consumer ", consumer_task); puts("\n");
			...
			if (msg_req_ptr){
				...
				} else { //Send a message delivery (remote consumer)
		...
		case READPIPE:
			...
			//puts("READPIPE - prod: "); puts(itoa(producer_task)); putsv(" consumer ", consumer_task); puts("\n");
...
int handle_packet(volatile ServiceHeader * p) {
	...
	//This case sincronizes the communication messages also in case of task migration, the migration allows several scenarios, that are handled inside this case
		...
		if (slot_ptr == 0){
			...
			//Test if the producer task is running at this processor, this conditions work together to the first by pass condition (above).
	...
	case  MESSAGE_DELIVERY:
		...
		msg_ptr = (Message *)(tcb_ptr->offset | tcb_ptr->reg[9]);
...
// void OS_SetCurrent(TCB *tcb) {
// 	current = tcb;
// 	ASM_SetCurrent(current);
// }
