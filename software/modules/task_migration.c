...
/**This function is the core of task migration.
...
void migrate_dynamic_memory(TCB * tcb_aux){
	...
	_stack_pointer = tcb_aux->reg[1]; // Modified: risc-v
...
void handle_migration_stack(volatile ServiceHeader * p, TCB * migrate_tcb){
	...
	if (p->stack_size > 0){
		DMNI_read_data(migrate_tcb->offset + migrate_tcb->reg[1], p->stack_size); //Modified RISC-V
	}
